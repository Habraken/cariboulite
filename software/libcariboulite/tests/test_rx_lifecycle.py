...
with tempfile.TemporaryDirectory(prefix='rx-lifecycle-') as directory:
    ...
    command = ['cc', '-O0', '-g', '-ffunction-sections', '-fdata-sections',
               ...
               str(Path(__file__).with_suffix('.c')), str(src/'nbfm_mod.c'), str(src/'nbfm_demod.c'), str(src/'alsa_source.c'), str(src/'alsa_sink.c'), str(src/'tone_source.c'), '-Wl,--gc-sections',
