...
with tempfile.TemporaryDirectory(prefix='rx-lifecycle-') as directory:
    ...
    wraps = ['at86rf215_read_buffer','nbfm_demod_create','cariboulite_radio_set_rx_sample_rate_flt','cariboulite_radio_set_tx_samp_cutoff_flt','cariboulite_radio_get_tx_samp_cutoff_flt','caribou_fpga_get_sys_ctrl_tx_sample_gap','cariboulite_radio_set_tx_power','malloc','calloc','free','cariboulite_radio_read_samples','pthread_create','pthread_cancel','pthread_join',
    ...
    command = ['cc', '-O0', '-g', '-ffunction-sections', '-fdata-sections',
               ...
               str(Path(__file__).with_suffix('.c')), str(src/'nbfm_mod.c'), str(src/'nbfm_demod.c'), str(src/'demod_worker.c'), str(src/'noise_squelch.c'), str(src/'carrier_squelch.c'), str(src/'pipeline_transport.c'), str(src/'pipeline_runtime.c'), str(src/'mod_worker.c'), str(src/'tx_pipeline.c'), str(src/'rx_pipeline.c'), str(src/'modem_selftest.c'), str(src/'alsa_source.c'), str(src/'alsa_sink.c'), str(src/'tone_source.c'), '-Wl,--gc-sections',
