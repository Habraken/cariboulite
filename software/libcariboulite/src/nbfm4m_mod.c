...
void nbfm4m_destroy(nbfm4m_mod_t* m){ if(!m)return; free(m->afifo); free(m); }
size_t nbfm4m_push_audio(nbfm4m_mod_t* m,const float* a,size_t N){
    size_t p=0; for(size_t n=0;n<N;n++){ if(m->cnt==m->cap)break;
        m->afifo[m->tail]=a[n]; m->tail=(m->tail+1)%m->cap; m->cnt++; p++; } return p;
}
static int fetch_audio(nbfm4m_mod_t* m){
    if(m->cnt==0)return 0; float x=m->afifo[m->head];
    m->head=(m->head+1)%m->cap; m->cnt--; if(x>1)x=1; else if(x<-1)x=-1;
    float xp=preemph_run(&m->pe,x); m->dphi_next=m->k*(double)xp; return 1;
}
// size_t nbfm4m_pull_iq(nbfm4m_mod_t* m, iq16_t* dst, size_t N){
//     size_t out=0; while(out<N){
//         if(m->interp_acc>=1.0){ m->dphi_cur=m->dphi_next;
//             if(!fetch_audio(m)) m->dphi_next=m->dphi_cur;
//             m->interp_step=m->lin? (m->dphi_next-m->dphi_cur)*m->a_to_rf : 0.0;
//             m->interp_acc=0.0;
//         }
//         double dphi=m->lin? (m->dphi_cur + m->interp_step*(m->interp_acc*m->R)) : m->dphi_cur;
//         m->phase+=dphi; if(m->phase>M_PI)m->phase-=2.0*M_PI; else if(m->phase<-M_PI)m->phase+=2.0*M_PI;
//         float ci=(float)cos(m->phase), sq=(float)sin(m->phase);
//         int32_t I=(int32_t)lrintf(ci*m->out_scale), Q=(int32_t)lrintf(sq*m->out_scale);
//         if(I>32767)I=32767; else if(I<-32768)I=-32768;
//         if(Q>32767)Q=32767; else if(Q<-32768)Q=-32768;
//         dst[out].i=(int16_t)I; dst[out].q=(int16_t)Q; out++; m->interp_acc+=m->a_to_rf;
//     } return out;
// }
